...
// 🚀 Setting this to >= 118 ensures we catch Sherrod Brown and other recently retired titans
const MIN_CONGRESS = 118;
...
export const api = {
    ...
    try {
      const query = `
        SELECT bioguide_id as id, icpsr, bioname as full_name, state_abbrev as state, 
               district_code, chamber, party_code, MAX(congress) as latest_congress
        FROM read_parquet('${VV_BASE}/HSall_members.parquet')
        WHERE congress >= ${MIN_CONGRESS}
          AND bioname ILIKE '%${searchQuery}%'
        GROUP BY id, icpsr, full_name, state, district_code, chamber, party_code
        ORDER BY latest_congress DESC, full_name ASC LIMIT 20
      `;
  ...
  async getVoteHistory(icpsr: number) {
    ...
    const conn = await db.connect(); // 🚀 Fixed: Removed .current() and unused variable
    try {
      const query = `
        ...
        WHERE CAST(v.icpsr AS INTEGER) = ${Math.floor(icpsr)}
          AND (rc.date LIKE '2024%' OR rc.date LIKE '2025%')
  ...
  async getDonationSummary(_icpsr: number, name: string) {
    ...
    try {
      ...
      const query = `
        ...
        WHERE UPPER("recipient.name") LIKE '%${lastName}%'
          AND (UPPER("recipient.name") LIKE '%${firstName}%' OR UPPER("recipient.name") LIKE '%${lastName}%')
