...
export function PoliticianSearchResults({ searchQuery, onSelectPolitician }: PoliticianSearchResultsProps) {
  ...
  if (!politicians || politicians.length === 0) return (
    <Card><CardContent className="pt-6 text-center text-muted-foreground py-8">No results found.</CardContent></Card>
  );
          ...
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-bold text-lg truncate">{politician.full_name}</h3>
                <Badge variant={getPartyInfo(politician.party).variant} className="text-[10px] font-mono">{getPartyInfo(politician.party).label}</Badge>
              </div>
              <div className="flex items-center text-sm text-muted-foreground gap-3 uppercase font-bold tracking-tighter opacity-60">
                <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{politician.state}</span>
                <span>•</span>
