...
// A vote where the losing side got under 10% was effectively uncontested —
// a donation "near" it carries no signal, since almost everyone voted the
// same way. Flagged so readers don't mistake calendar noise for influence.
const isNearUnanimous = (item: CorrelatedDonation) => {
  if (item.yea_count === null || item.nay_count === null) return false;
  const total = item.yea_count + item.nay_count;
  if (total === 0) return false;
  return Math.min(item.yea_count, item.nay_count) / total < 0.1;
};
...
export function CorrelatedTimeline({
  ...
  const [hideUnanimous, setHideUnanimous] = useState<boolean>(false);
    ...
    <div className="space-y-6 rounded-3xl border border-white/10 !bg-zinc-900/90 p-6 shadow-2xl shadow-black/80">
      ...
      <div className="grid grid-cols-1 gap-4 rounded-xl border border-white/5 bg-white/[0.01] p-4 lg:grid-cols-12">
        ...
        <div className="relative lg:col-span-5">
          <Search
            className="absolute top-3 left-3 h-4 w-4 text-white/30"
            aria-hidden="true"
          />
          <input
            type="text"
            aria-label="Search bills, corporate donors, or PAC names"
            placeholder="Search bills, corporate donors, PAC names... THEN PRESS ENTER"
            value={search}
            onChange={(e) => {
              setSearch(e.target.value);
            }}
            className="focus:border-primary/50 w-full rounded-xl border border-white/10 bg-zinc-950 py-2.5 pr-4 pl-10 text-sm text-white placeholder-white/50 transition outline-none focus-visible:ring-2 focus-visible:ring-[#4A90E2]"
          />
        </div>
        ...
        <div className="lg:col-span-3">
          <select
            ...
            className="focus:border-primary/50 w-full rounded-xl border border-white/10 bg-zinc-950 p-2.5 text-sm text-white transition outline-none focus-visible:ring-2 focus-visible:ring-[#4A90E2]"
          >
        ...
        <div className="lg:col-span-2">
          <select
            aria-label="Filter by donation timing relative to vote"
            value={direction}
            onChange={(e) => {
        ...
        <div className="flex items-center gap-3 pt-1 lg:col-span-12">
          <label className="relative inline-flex cursor-pointer items-center">
            <input
              type="checkbox"
              aria-label="Hide major party leadership transfers"
              checked={hidePacs}
              onChange={(e) => {
            ...
            />
            <div className="peer aftermarket h-5 w-9 rounded-full bg-zinc-800 peer-checked:bg-emerald-500 peer-focus-visible:ring-2 peer-focus-visible:ring-[#4A90E2] after:absolute after:top-[2px] after:left-[2px] after:h-4 after:w-4 after:rounded-full after:bg-zinc-400 after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:bg-white"></div>
          </label>
        ...
        {/* Near-unanimous vote suppressor: a donation near a 424-2 vote is
            calendar noise, not signal */}
        <div className="flex items-center gap-3 pt-1 lg:col-span-12">
          <label className="relative inline-flex cursor-pointer items-center">
            <input
              type="checkbox"
              aria-label="Show contested votes only"
              checked={hideUnanimous}
              onChange={(e) => {
                setHideUnanimous(e.target.checked);
              }}
              className="peer sr-only"
            />
            <div className="peer h-5 w-9 rounded-full bg-zinc-800 peer-checked:bg-emerald-500 peer-focus-visible:ring-2 peer-focus-visible:ring-[#4A90E2] after:absolute after:top-[2px] after:left-[2px] after:h-4 after:w-4 after:rounded-full after:bg-zinc-400 after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:bg-white"></div>
          </label>
          <span className="text-xs font-medium text-white/60">
            Show contested votes only (hides near-unanimous votes, where
            donation timing carries no signal)
          </span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        >
          <Loader2
            className="text-primary mb-2 h-8 w-8 animate-spin"
            aria-hidden="true"
          />
        ...
        <>
          <div
            className="custom-scrollbar max-h-[600px] space-y-4 overflow-y-auto pr-2 focus-visible:ring-2 focus-visible:ring-[#4A90E2]"
            role="region"
            aria-label="Vote and donation correlation results"
            tabIndex={0}
          >
            {data.map((item, index) => (
              <div
              ...
              >
                ...
                <div className="space-y-1.5 lg:col-span-5">
                  <div className="flex items-center gap-2">
                    ...
                    {item.yea_count !== null && item.nay_count !== null && (
                      <span className="rounded bg-white/5 px-1.5 py-0.5 font-mono text-[10px] text-white/60">
                        {item.yea_count}–{item.nay_count}
                      </span>
                    )}
                    {isNearUnanimous(item) && (
                      <span
                        className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] font-semibold text-white/60"
                        title="This vote was nearly unanimous — donation timing near it carries no signal"
                      >
                        Near-unanimous
                      </span>
                    )}
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-3 lg:col-span-5 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-4">
                  <div className="space-y-1">
                    <p className="text-sm font-bold tracking-tight text-white">
                      ...
                      {item.cmte_id && (
                        <a
                          href={`https://www.fec.gov/data/committee/${item.cmte_id}/`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="ml-2 align-middle text-[10px] font-semibold tracking-wide text-[#4A90E2] uppercase hover:underline focus-visible:ring-2 focus-visible:ring-[#4A90E2] focus-visible:outline-none"
                          aria-label={`Verify ${item.donor_name} at FEC.gov (opens in new tab)`}
                        >
                          Verify at FEC.gov ↗
                        </a>
                      )}
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-semibold tracking-wider text-[#4A90E2] uppercase">
                        {item.sector}
                      </span>
                      <span
